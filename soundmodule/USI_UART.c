...
*
* Supported devices : ATtiny26 - changing to support ATTiny2313
*
...
*
*	v1 - works on AT-Tiny2313 in Xmit mode only.  Rx doesn't work.
*	Had to disable the Pin Change interupts to keep from crashing.
*	works with both 1-Mhz and 8-Mhz system clock. (with changes to config file)
*
****************************************************************************/
...
// __no_init __regvar static unsigned char USI_UART_TxData @ 15;   // Tells the compiler to store the byte to be transmitted in register.
...
static volatile unsigned char UART_RxBuf[UART_RX_BUFFER_SIZE];  // UART buffers. Size is definable in the header file.
...
void USI_UART_Initialise_Transmitter( void )                              
{
    ...
    TCNT0  = 0x00;
    GTCCR = 1;											// Reset the prescaler
//    TCCR0B = (0<<CS02)|(0<<CS01)|(1<<CS00);				// Start Timer0. (no-prescaler)
    TCCR0B = (0<<CS02)|(1<<CS01)|(0<<CS00);				// Start Timer0. (clk/8)
    ...
    DDRB  |= (1<<PB6);                                        // Configure USI_DO as output.
...
{  
    PORTB |=   (1<<PB5)|(1<<PB6)|(1<<PB7);         // Enable pull up on USI DO, DI and SCK pins. (And PB3 because of pin change interrupt)   
    DDRB  &= ~((1<<PB5)|(1<<PB6)|(1<<PB7));        // Set USI DI, DO and SCK pins as inputs.  
    USICR  =  0;                                            // Disable USI.
    EIFR   =  (1<<PCIF);                                    // Clear pin change interrupt flag.
//    PCMSK = (1<<PCINT5);
//	GIMSK |=  (1<<PCIE);                                    // Enable pin change interrupt for PB5.
}
...
// The pin change (PortB) interrupt is used to detect USI_UART reception.
// It is here the USI is configured to sample the UART signal.
/*
SIGNAL(SIG_PCINT)
{                                                                    
    if (!( PINB & (1<<PB5) ))                                     // If the USI DI pin is low, then it is likely that it
    ...
    if( USI_UART_status.ongoing_Transmission_From_Buffer )      
    {
        ...
        else
        {
            ...
            else
            {
                ...
                PORTB |=   (1<<PB7)|(1<<PB6)|(1<<PB5);         // Enable pull up on USI DO, DI and SCK pins. (And PB3 because of pin change interrupt)   
                DDRB  &= ~((1<<PB7)|(1<<PB6)|(1<<PB5));        // Set USI DI, DO and SCK pins as inputs.  
                USICR  =  0;                                            // Disable USI.
                EIFR   =  (1<<PCIF);                                    // Clear pin change interrupt flag.
              //  PCMSK = (1<<PCINT5);
			//	GIMSK |=  (1<<PCIE);                                   // Enable pin change interrupt for PB5.
