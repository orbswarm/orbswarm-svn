...
----------------------
	...
	Shaft	PortD 2	        Geartooth sense in    DIP 4
...
void Init_Chip(void)
{
  ...
  PORTD |= _BV(PD2); 		/* ensure pullup on for interrupts */
  PORTD |= _BV(PD3);
  ...
  Killswitch_Init();  	/* enable interrupts on killsw input */
...
int main (void)
{
  ...
  for (;;) {	// loop forever
    ...
    // Heart-beat is foreground -- true indication prog is alive.
    // also service killswitch interrupts and fail-safe
    check_heart_beat( &state ); 
    ...
    // do these tasks only 10 times per second
    if (Timer0_10hz_Flag) { 
      Timer0_10hz_Flag = 0;
      Fail_Safe_Counter++;
      
      Encoder_sample_speed(MOTOR1_SHAFT_ENCODER); 
...
// Check kill switch if set

void check_heart_beat(unsigned char *state)
{
  //	short theData;
  
  // check for killswicth interrupt

  if (Killswitch_Query()) {
    putstr("KILLSWITCH EVENT\n");
    Set_Motor1_PWM( 0, FORWARD );
  ...
  if (Timer0_ticks > 511) {	//  512 tics per second - heart-beat LED
    ...
    if (*state == 0) { // toggle hearbeat LED
      turn_LED(HB_LED,ON);
      *state = 1;
    }
    else {
    ...
    if (Fail_Safe && (Fail_Safe_Counter > 15) && ENABLE_FAIL_SAFE) 
      {	
