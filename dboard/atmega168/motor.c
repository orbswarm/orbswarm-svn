...
#define MOTOR1_FORWARD()  PORTD &= ~_BV(PIND4); TCCR1A &= ~_BV(COM1B0); TCCR1A |= _BV(COM1B1)
#define MOTOR1_REVERSE()  PORTD |= _BV(PIND4); TCCR1A |= (_BV(COM1B0) | _BV(COM1B1))
#define MOTOR1_DISABLE() PORTD |= _BV(PIND5)
#define MOTOR1_ENABLE()  PORTD &= ~_BV(PIND5)
#define MOTOR1_BRAKE()   TCCR1A &= ~_BV(COM1B1)

#define MOTOR2_FORWARD() PORTD &= ~_BV(PIND6); TCCR1A &= ~_BV(COM1A0); TCCR1A |= _BV(COM1A1)
#define MOTOR2_REVERSE() PORTD |= _BV(PIND6); TCCR1A |= (_BV(COM1A0) | _BV(COM1A1))
#define MOTOR2_DISABLE() PORTD |= _BV(PIND7)
#define MOTOR2_ENABLE()  PORTD &= ~_BV(PIND7)
