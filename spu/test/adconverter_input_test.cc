...
int startupADC() {
	...
	dr_page = (unsigned long)mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, devmem, DATA_PAGE);
	assert(&dr_page != MAP_FAILED);
	
	spistart = (unsigned long)mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, devmem, SPI_PAGE);
	...
	pld_page = (unsigned long)mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, devmem, PLD_PAGE);
