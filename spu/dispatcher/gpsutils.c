...
int parseGPSGGASentence(swarmGpsData * gpsdata) {
    ...
    if (strcmp(gpsdata->gpsSentenceType, SWARM_NMEA_GPS_SENTENCE_TYPE_GPGGA)
            == 0) {
...
char UTMLetterDesignator(double Lat) {
    ...
    if ((84 >= Lat) && (Lat >= 72))
        ...
        LetterDesignator = 'Z'; //error flag indicating that the Latitude is outside the UTM limits

    return LetterDesignator;
}

void decimalLatLongtoUTM(const double ref_equ_radius,
        const double ref_ecc_squared, swarmGpsData * gpsdata) {
    ...
    M = a * ((1 - eccSquared / 4 - 3 * eccSquared * eccSquared / 64 - 5
            * eccSquared * eccSquared * eccSquared / 256) * LatRad - (3
            * eccSquared / 8 + 3 * eccSquared * eccSquared / 32 + 45
            * eccSquared * eccSquared * eccSquared / 1024) * sin(2 * LatRad)
            + (15 * eccSquared * eccSquared / 256 + 45 * eccSquared
                    * eccSquared * eccSquared / 1024) * sin(4 * LatRad) - (35
            * eccSquared * eccSquared * eccSquared / 3072) * sin(6 * LatRad));

    gpsdata->UTMEasting = (double) (k0 * N * (A + (1 - T + C) * A * A * A / 6
            + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A
                    * A * A / 120) + 500000.0);

    gpsdata->UTMNorthing = (double) (k0 * (M + N * tan(LatRad) * (A * A / 2
            + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24 + (61 - 58 * T
            + T * T + 600 * C - 330 * eccPrimeSquared) * A * A * A * A * A * A
            / 720)));
    if (Lat < 0)
    ...
    if (status == SWARM_SUCCESS) {
        ...
        if (status == SWARM_SUCCESS) {
            logit(eGpsLog, eLogDebug,
                    ...
                    gpsdata->londd, gpsdata->nmea_utctime);
